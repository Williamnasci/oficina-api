...
Identificados numa revisão externa do CI/CD e da infraestrutura da Lambda de autenticação (`oficina-lambda-auth`). Assumidos deliberadamente dado o perfil do projeto (conta pessoal Free Tier, sem dado real de usuário, um único mantenedor com acesso root) — não valem o esforço de correção neste escopo, mas ficam registrados aqui:

- **Credenciais AWS estáticas (`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`) disponíveis aos jobs de `terraform plan` em todos os três repositórios de infraestrutura** (`oficina-infra-database`, `oficina-infra-k8s`, `oficina-lambda-auth`). O ideal em produção seria GitHub OIDC com role de plan somente-leitura e role de apply protegida por `environment: production`, eliminando chaves de longa duração do GitHub. Não migrado porque exigiria mudar os três repositórios de forma consistente e não há PRs de fork nem múltiplos colaboradores neste projeto.
- **Policy IAM `oficina-terraform-scoped` gerenciada manualmente via AWS CLI (usuário root), não versionada em Terraform.** As revisões de permissão (ex.: `lambda:CreateFunction` exige `Resource: "*"` por não suportar permissão em nível de recurso) foram aplicadas diretamente via `aws iam create-policy-version`, então a policy vigente não é reprodutível a partir do código do repositório. Colocar essa policy sob Terraform esbarra num problema de bootstrap (a policy que dá permissão ao Terraform não pode ser criada pelo próprio Terraform sem uma etapa de bootstrap separada com credenciais mais privilegiadas) — não implementado por não valer o esforço para um único mantenedor.
- **RDS aceita conexões de `0.0.0.0/0` na porta 5432** para viabilizar o acesso da Lambda de autenticação, que roda fora da VPC. Já documentado e justificado no [ADR-0006](adr/0006-acesso-rede-rds-fora-vpc.md) — mitigado por TLS obrigatório (`rds.force_ssl`) e senha forte, não por isolamento de rede. A alternativa correta (Lambda dentro da VPC + NAT Gateway ou VPC endpoint do Secrets Manager) tem custo real e não é Free Tier.
