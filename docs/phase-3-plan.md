...
| Banco de dados gerenciado | `oficina-infra-database` — RDS PostgreSQL (Free Tier: `db.t3.micro`, 20GB) | Não (hoje é StatefulSet Postgres em K8s) |
| Cluster Kubernetes com escalabilidade | Kind local (decisão final — ver `docs/adr/0003-cluster-kubernetes-local.md`), HPA já existe na Fase 2 | Parcial — falta HPA validado contra o novo fluxo |
